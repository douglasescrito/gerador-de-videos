import { listCommands, PROTECTED_COMMAND_IDS } from "./command-registry.mjs";
import { ERROR_CODES } from "./cli-errors.mjs";

export const AGENT_CONTRACT_SCHEMA = "mkt-videos/agent-contract@1";

function inlineCode(value) {
  return `\`${String(value)}\``;
}

export function renderAgentContractDocumentation() {
  const commands = listCommands();
  const conditionalConfirmations = commands.flatMap((entry) =>
    entry.conditionalConfirmations.map((rule) => ({ command: entry.id, ...rule }))
  );
  const knowledgeActions =
    commands.find((entry) => entry.id === "knowledge")?.actionValues ?? [];
  const errorCodes = Object.values(ERROR_CODES).sort();
  ...
  const lines = [
    ...
    "  ```",
    "",
    `- \`code\` reconhecidos hoje: ${errorCodes.map(inlineCode).join(", ")}.`,
    ...
    "| Comando | Confirmação |",
    "| --- | --- |",
    ...conditionalConfirmations.map((rule) =>
      `| ${inlineCode(`${rule.command} ${rule.when}`)} | ${inlineCode(rule.flag)} |`
    ),
    "",
    ...
    "",
    `Ações aceitas: ${knowledgeActions.map(inlineCode).join(", ")}.`,
    ...
    "",
    `${PROTECTED_COMMAND_IDS.map(inlineCode).join(", ")} têm teste de regressão dedicado`,
